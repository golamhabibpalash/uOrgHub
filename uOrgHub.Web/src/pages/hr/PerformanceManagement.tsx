      ...
      <Modal title={activeTab === "cycles" ? "Add Review Cycle" : activeTab === "goals" ? "Add Goal" : activeTab === "reviews" ? "Add Review" : trainingMode === "enroll" ? "Enroll Employee" : "Add Training Program"} open={modal} onClose={() => setModal(false)}>
        {activeTab === "cycles" && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-gray-500 mb-1 block">Start Date</label><DateInput className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={cycleForm.startDate} onChange={e => setCycleForm(f => ({ ...f, startDate: e.target.value }))} /></div><div><label className="text-xs text-gray-500 mb-1 block">End Date</label><DateInput className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={cycleForm.endDate} onChange={e => setCycleForm(f => ({ ...f, endDate: e.target.value }))} /></div></div>
        ...
        {activeTab === "goals" && (
          <div className="space-y-3">
            ...
            <div><label className="text-xs text-gray-500 mb-1 block">Target Date</label><DateInput className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={goalForm.targetDate} onChange={e => setGoalForm(f => ({ ...f, targetDate: e.target.value }))} /></div>
        ...
        {activeTab === "training" && trainingMode === "add" && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3"><div><label className="text-xs text-gray-500 mb-1 block">Start</label><DateInput className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={programForm.startDate} onChange={e => setProgramForm(f => ({ ...f, startDate: e.target.value }))} /></div><div><label className="text-xs text-gray-500 mb-1 block">End</label><DateInput className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={programForm.endDate} onChange={e => setProgramForm(f => ({ ...f, endDate: e.target.value }))} /></div><div><label className="text-xs text-gray-500 mb-1 block">Max</label><input type="number" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={programForm.maxParticipants} onChange={e => setProgramForm(f => ({ ...f, maxParticipants: Number(e.target.value) }))} /></div></div>
