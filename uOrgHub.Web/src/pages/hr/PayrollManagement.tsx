...
export default function PayrollManagement() {
  ...
  const [gradeForm, setGradeForm] = useState({ name: "", code: "", description: "", minSalary: 0, maxSalary: 0 });
  const [compForm, setCompForm] = useState({ name: "", code: "", type: "Allowance", isTaxable: true });
  const [cycleForm, setCycleForm] = useState({ name: "", startDate: "", endDate: "", paymentDate: "" });
  ...
  function openModal(tab: typeof activeTab) {
    setActiveTab(tab);
    setModal(true);
    if (tab === "grades") setGradeForm({ name: "", code: "", description: "", minSalary: 0, maxSalary: 0 });
    if (tab === "components") setCompForm({ name: "", code: "", type: "Allowance", isTaxable: true });
    if (tab === "cycles") { setCycleForm({ name: "", startDate: "", endDate: "", paymentDate: "" }); setEditing(null); }
    if (tab === "expenses") setExpForm({ employeeId: "", amount: 0, category: "", description: "" });
  }

  const gradeCols = [{ key: "code", label: "Code" }, { key: "name", label: "Grade Name" }, { key: "minSalary", label: "Min Salary" }, { key: "maxSalary", label: "Max Salary" }];
  const compCols = [{ key: "code", label: "Code" }, { key: "name", label: "Component Name" }, { key: "type", label: "Type" }, { key: "isTaxable", label: "Taxable", render: (r: SalaryComponent) => <span className={`text-xs px-2 py-0.5 rounded-full ${r.isTaxable ? "bg-yellow-50 text-yellow-700" : "bg-gray-50 text-gray-600"}`}>{r.isTaxable ? "Yes" : "No"}</span> }];
  const cycleCols = [{ key: "name", label: "Cycle Name" }, { key: "startDate", label: "Start", render: (r: PayrollCycle) => new Date(r.startDate).toLocaleDateString() }, { key: "endDate", label: "End", render: (r: PayrollCycle) => new Date(r.endDate).toLocaleDateString() }, { key: "paymentDate", label: "Payment Date", render: (r: PayrollCycle) => new Date(r.paymentDate).toLocaleDateString() }, { key: "status", label: "Status", render: (r: PayrollCycle) => <span className={`text-xs px-2 py-0.5 rounded-full ${r.status === "Active" ? "bg-green-50 text-green-700" : "bg-gray-50 text-gray-600"}`}>{r.status}</span> }];
      ...
      <Modal title={activeTab === "grades" ? "Add Salary Grade" : activeTab === "components" ? "Add Salary Component" : activeTab === "cycles" ? "Add Payroll Cycle" : "Submit Expense"} open={modal} onClose={() => setModal(false)}>
        ...
        {activeTab === "components" && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-gray-500 mb-1 block">Type</label><select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={compForm.type} onChange={e => setCompForm(f => ({ ...f, type: e.target.value }))}><option value="Allowance">Allowance</option><option value="Deduction">Deduction</option></select></div><div><label className="text-xs text-gray-500 mb-1 block">Taxable</label><select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={String(compForm.isTaxable)} onChange={e => setCompForm(f => ({ ...f, isTaxable: e.target.value === "true" }))}><option value="true">Yes</option><option value="false">No</option></select></div></div>
