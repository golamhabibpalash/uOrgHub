      ...
      <Modal title={activeTab === "postings" ? "Add Job Posting" : activeTab === "candidates" ? "Add Candidate" : activeTab === "applications" ? "Add Application" : "Schedule Interview"} open={modal} onClose={() => setModal(false)}>
        {activeTab === "postings" && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-gray-500 mb-1 block">Status</label><select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={postingForm.status} onChange={e => setPostingForm(f => ({ ...f, status: e.target.value }))}><option value="Published">Published</option><option value="Closed">Closed</option><option value="OnHold">On Hold</option></select></div><div><label className="text-xs text-gray-500 mb-1 block">Closing Date</label><DateInput className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={postingForm.closingDate} onChange={e => setPostingForm(f => ({ ...f, closingDate: e.target.value }))} /></div></div>
        ...
        {activeTab === "interviews" && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-gray-500 mb-1 block">Date/Time</label><DateInput mode="datetime" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={interviewForm.scheduledAt} onChange={e => setInterviewForm(f => ({ ...f, scheduledAt: e.target.value }))} /></div><div><label className="text-xs text-gray-500 mb-1 block">Location</label><input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={interviewForm.location} onChange={e => setInterviewForm(f => ({ ...f, location: e.target.value }))} /></div></div>
