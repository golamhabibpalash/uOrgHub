      ...
      <Modal title={activeTab === "postings" ? "Add Job Posting" : activeTab === "candidates" ? "Add Candidate" : activeTab === "applications" ? "Add Application" : "Schedule Interview"} open={modal} onClose={() => setModal(false)}>
        ...
        {activeTab === "applications" && (
          <div className="space-y-3">
            ...
            <div><label className="text-xs text-gray-500 mb-1 block">Job Posting</label><select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={appForm.jobPostingId} onChange={e => setAppForm(f => ({ ...f, jobPostingId: e.target.value }))}><option value="">Select</option>{postings.filter(p => p.status === "Open").map(p => <option key={p.id} value={p.id}>{p.title}</option>)}</select></div>
