...
public class InvoicesController : BaseController
{
    ...
    public InvoicesController(IMediator mediator, IExportService exportService)
    {
        _mediator = mediator;
    ...
    [HttpGet("export")]
    [RequireClaim(Claims.Accounts.Invoices.Export)]
    public async Task<IActionResult> Export([FromQuery] string format = "xlsx")
    {
        var data = await _mediator.Send(new GetAllInvoicesForExportQuery());
        var fmt = format.ToLower() switch { "csv" => ExportFormat.Csv, _ => ExportFormat.Xlsx };
        var result = await _exportService.ExportAsync(data, InvoiceExportColumns.Get(), new ExportOptions
        {
            Format = fmt,
            EntityName = "Invoices"
        });
