...
public class GetStockTransactionsQueryHandler : IRequestHandler<GetStockTransactionsQuery, PagedResult<StockTransactionResponseDto>>
{
    ...
    public async Task<PagedResult<StockTransactionResponseDto>> Handle(GetStockTransactionsQuery request, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Request.Search))
            query = query.WhereSearch(request.Request.Search, x => x.TransactionNumber, x => x.ReferenceNumber);

        query = query.ApplySorting(request.Request.SortBy ?? "TransactionDate", request.Request.SortDescending);
...
public record GetAllStockTransactionsForExportQuery(
    Guid? WarehouseId = null, Guid? ItemVariantId = null, StockTransactionStatus? Status = null)
    : IRequest<List<StockTransactionResponseDto>>;

public class GetAllStockTransactionsForExportQueryHandler
    : IRequestHandler<GetAllStockTransactionsForExportQuery, List<StockTransactionResponseDto>>
{
    private readonly AppDbContext _context;
    public GetAllStockTransactionsForExportQueryHandler(AppDbContext context) => _context = context;

    public async Task<List<StockTransactionResponseDto>> Handle(GetAllStockTransactionsForExportQuery request, CancellationToken ct)
    {
        var query = _context.Set<Models.Entities.StockTransaction>()
            .Include(x => x.ItemVariant)
            .Include(x => x.Warehouse)
        ...
        var items = await query.OrderByDescending(x => x.TransactionDate).ToListAsync(ct);
